...
import Login from './Login'; // Adjust the path as necessary if Login.js is in a different folder.
...
const App = () => {
 ...
 const onLogin = (username, password) => {
   ...
   if (username === "admin" && password === "password") {
     setIsAuthenticated(true);  // If login is successful, set authenticated to true
   } else {
 ...
 useEffect(() => {
   ...
   client.on("message", (topic, message) => {
     if (topic === MQTT_TOPIC_DOOR_MOTION) {
       ...
       setDoorMotionDetected(motionData.motion_detected);
     }


     if (topic === MQTT_TOPIC_WINDOW_MOTION) {
       const motionData = JSON.parse(message.toString());
       setWindowMotionDetected(motionData.motion_detected);
     }
     ...
     if (topic === MQTT_TOPIC_DOOR_CAMERA_IMAGE) {
       setDoorCameraImage(message.toString());
       setShowDoorImage(true);
     ...
     if (topic === MQTT_TOPIC_WINDOW_CAMERA_IMAGE) {
     ...
     if (topic === MQTT_TOPIC_LIGHT) {
       setLightOn(JSON.parse(message.toString()).light_on);
     }


     if (topic === MQTT_TOPIC_SMOKE) {
